...
import { availablePuppies as schnauzerPuppies } from "../data/schnauzerPuppies";
import { availablePuppies as shibaPuppies } from "../data/shibaPuppies";
import { BreedProps } from "../types";

const Contact = ({ breed }: BreedProps) => {
    ...
    // If the breed in URL doesn't match the current breed, redirect to the correct breed's contact page
    useEffect(() => {
        if (urlBreed && urlBreed !== breed) {
            navigate(`/contact?puppy=${selectedPuppyFromUrl}&breed=${breed}`);
        }
    }, [urlBreed, breed, selectedPuppyFromUrl, navigate]);
    ...
    useEffect(() => {
        if (selectedPuppyFromUrl) {
            const decodedPuppyName = decodeURIComponent(selectedPuppyFromUrl);
            setFormData((prev) => ({
                ...prev,
                preferredPuppy: decodedPuppyName,
            }));
        }
